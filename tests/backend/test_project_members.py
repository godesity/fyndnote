...
def test_candidates_require_manager(client):
    ...
    client.put(
        ...
        json={"user_id": "bob", "role": "annotator", "actor": "alice"},
    )
    # bob is a plain annotator -> may not even enumerate candidates
    assert (
        client.get(f"/api/v1/projects/{pid}/member-candidates?user_id=bob").status_code
        == 403
    )
...
def test_add_member_then_they_can_see_project(client):
    ...
    assert pid in [
        p["id"] for p in client.get("/api/v1/projects?user_id=bob").json()["projects"]
    ]
...
def test_reassigning_role_updates_instead_of_duplicating(client):
    ...
    client.put(
        f"/api/v1/projects/{pid}/members",
        json={"user_id": "bob", "role": "project_admin", "actor": "alice"},
    )
    members = client.get(f"/api/v1/projects/{pid}/members?user_id=alice").json()
...
def test_project_admin_can_manage_members_without_global_admin(client):
    ...
    # carol must exist: the route 404s on an unknown user_id.
    _ensure_user("carol", "Carol")
    client.put(
        f"/api/v1/projects/{pid}/members",
        json={"user_id": "bob", "role": "project_admin", "actor": "alice"},
    ...
    resp = client.put(
        f"/api/v1/projects/{pid}/members",
        json={"user_id": "carol", "role": "annotator", "actor": "bob"},
    )
    assert resp.status_code == 200
...
def test_add_member_rejects_unknown_user(client):
    ...
    resp = client.put(
        f"/api/v1/projects/{pid}/members",
        json={"user_id": "nobody", "role": "annotator", "actor": "alice"},
    )
...
def test_add_member_rejects_unknown_role(client):
    ...
    resp = client.put(
        f"/api/v1/projects/{pid}/members",
        json={"user_id": "bob", "role": "owner", "actor": "alice"},
    )
...
def test_annotator_cannot_manage_members(client):
    ...
    resp = client.put(
        f"/api/v1/projects/{pid}/members",
        json={"user_id": "carol", "role": "project_admin", "actor": "bob"},
    )
    assert resp.status_code == 403
    assert "carol" not in [
        m["user_id"]
        for m in client.get(f"/api/v1/projects/{pid}/members?user_id=alice").json()
    ]


def test_member_routes_404_for_unknown_project(client):
    resp = client.put(
        "/api/v1/projects/nope/members",
        json={"user_id": "bob", "role": "annotator", "actor": "alice"},
    )
    assert resp.status_code == 404
...
def test_remove_member(client):
    ...
    assert (
        client.delete(f"/api/v1/projects/{pid}/members/bob?user_id=alice").status_code
        == 404
    )
    assert pid not in [
        p["id"] for p in client.get("/api/v1/projects?user_id=bob").json()["projects"]
...
def test_last_project_admin_cannot_be_removed_by_a_peer(client):
    ...
    assert (
        client.delete(f"/api/v1/projects/{pid}/members/alice?user_id=bob").status_code
        == 200
    )
    ...
    assert [
        m["user_id"]
        for m in client.get(f"/api/v1/projects/{pid}/members?user_id=bob").json()
    ] == ["bob"]
...
def test_global_admin_may_remove_last_project_admin(client):
    ...
    assert (
        client.delete(f"/api/v1/projects/{pid}/members/alice?user_id=alice").status_code
        == 200
    )
...
def test_update_project_requires_manager(client):
    ...
    assert (
        client.put(
            f"/api/v1/projects/{pid}?user_id=bob", json={"name": "hijacked"}
        ).status_code
        == 403
    )
    ...
    assert (
        client.put(
            f"/api/v1/projects/{pid}?user_id=alice", json={"name": "renamed"}
        ).status_code
        == 200
    )
...
def test_project_detail_exposes_role_flags(client):
    ...
    assert (
        as_annotator["my_role"],
        as_annotator["can_manage"],
        as_annotator["can_view"],
    ) == (
        "annotator",
        False,
        True,
