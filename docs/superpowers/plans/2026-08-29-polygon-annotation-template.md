# Polygon Annotation Template — Implementation Plan

Date: 2026-08-29
Spec: `docs/superpowers/specs/2026-08-29-polygon-annotation-template-design.md`

## Overview

Add a `PolygonField` widget supporting closed / open / point shapes, wire it
into `widgets/index.ts`, add an "Image Polygon" predefined template, and add a
`WidgetDocs` entry. All changes are frontend-only; polygons are ordinary
annotation JSON values under a widget `name`.

## Task 1: Create `frontend/src/widgets/PolygonField.tsx`

New component mirroring `BBoxField` conventions (color palette, `getColor`,
`registerField`). Full implementation below.
...
- Single click adds a draft point; a second click within 250ms finishes the shape (no double point added).
- `Enter` finishes the draft.
- Point mode adds an instant point shape.
- Vertex click deletes the vertex; shape body click deletes the shape.
- Vertex drag moves that vertex; shape drag moves the whole shape (offset anchored to the shape's first point).
- Draft preview line follows the cursor; rejected drafts (below minimum) are simply not committed.

## Task 2: Export from `frontend/src/widgets/index.ts`

Add `export { default as PolygonField } from './PolygonField';` alongside the
existing `BBoxField` export.

## Task 3: Add "Image Polygon" to `frontend/src/predefinedTemplates.ts`

Mirror the existing `IMAGE_BBOX` entry shape (group `image`, `data.image_url`).
...
const IMAGE_POLYGON = `<div>
  ...
  {data.image_url && (
    <PolygonField
      ...
      defaultValue={[
        { id: "demo-closed", category: "cat", type: "closed", points: [
          { x: 0.1, y: 0.1 }, { x: 0.3, y: 0.2 }, { x: 0.25, y: 0.4 }, { x: 0.1, y: 0.35 }
        ]},
        { id: "demo-open", category: "dog", type: "open", points: [
          { x: 0.5, y: 0.1 }, { x: 0.6, y: 0.3 }, { x: 0.7, y: 0.2 }
        ]},
        { id: "demo-point", category: "person", type: "point", points: [{ x: 0.8, y: 0.8 }] },
      ]}
    />
  )}
...
Add a `PREDEFINED_TEMPLATES` entry:
```ts
{
  name: 'Image Polygon',
...
```
(Keep `annotations` values consistent with `defaultValue` in the template.)

## Task 4: Add `PolygonField` to `frontend/src/components/WidgetDocs.tsx`

Add an entry to the `WIDGETS` array:
```ts
{
  name: 'PolygonField',
...
```
Match the exact object shape used by existing entries (verify against `BBoxField` entry when editing).

## Task 5: Verify

No frontend test framework. Verification commands from `frontend/`:
- `npx tsc --noEmit` — must pass (also `npx tsc -b`).
- `npm run lint` — must pass.
- Manual: run `npm run dev`, open SetupView and EditProjectView, select **Image Polygon** in the Load Template dialog, confirm preview renders the 3 demo shapes and drawing works (add closed/open/point; drag vertex; drag shape; click vertex to delete; click body to delete; Enter/double-click finish).

## Files

- `frontend/src/widgets/PolygonField.tsx` (new)
- `frontend/src/widgets/index.ts` (modify)
- `frontend/src/predefinedTemplates.ts` (modify)
- `frontend/src/components/WidgetDocs.tsx` (modify)

Note: the uncommitted prediction-filter work in the working tree is unrelated and must not be included in this feature's commits.
