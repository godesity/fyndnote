...
def prefill_row(pid: str, row_index: int) -> dict:
    ...
    db.execute(
        "INSERT OR REPLACE INTO fyndnote_ml_annotations (project_id, row_index, annotator, data, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
...
def batch_prefill(pid: str, row_indices: list[int] | None = None) -> dict:
    ...
    existing = {
        r[0]
        for r in db.execute(
            "SELECT row_index FROM fyndnote_ml_annotations WHERE project_id = ?", (pid,)
        ).fetchall()
    }
    db.close()
